...
void *readtx(void *arg){
    ...
    int tid = node->tid;
    int obno = node->obno;
    int obCnt = node->count;
    int sgno;

    start_operation(tid, node->count); 

    // get existing transaction
    zgt_tx *txPt = get_tx(tid);

    if (txPt != NULL) {
      sgno = txPt->sgno;
      txPt->set_lock(tid, sgno, obno, obCnt, 'S');    // call method for handling lock request
    }
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  if (txPt != NULL) {
    sgno = txPt->sgno;
    txPt->set_lock(tid, sgno, obno, obCnt, 'X');    // call method for handling lock request
  }
...
void *aborttx(void *arg)
{
  ...
  zgt_tx *txPt = get_tx(tid);     // get existing transaction

  if (txPt != NULL) {
    do_commit_abort(tid, 'A');    // call method for handlling abort
    open_logfile_for_append();
    fprintf(logfile, "T%-3d\t\tAbortTx  \n", tid);
    fflush(logfile);
  }   
  zgt_v(0); 			// Release tx manager 
...
void *committx(void *arg)
{
  //remove the locks before committing
    ...
    if (txPt != NULL) {
      do_commit_abort(tid, 'E');    // call method for handlling commit
      open_logfile_for_append();
      fprintf(logfile, "T%-3d\t\tCommitTx  \n", tid);
      fflush(logfile);
    }   
    zgt_v(0); 			// Release tx manager 
...
void *do_commit_abort(long t, char status){
    ...
    zgt_tx *txPt = get_tx(t);
    txPt->status = status;

    txPt->free_locks();   // free all locks

    if (status == 'E')    // if commit operation
      txPt->remove_tx();    //remove the transaction from the TM

    int curSemNo = txPt->semno;
    int totalWaitingTx;

    if (curSemNo != -1) {   // if any other transactions waiting on this
        totalWaitingTx = zgt_nwait(txPt->semno);
        while(totalWaitingTx--)   // wakes up waiting transactions
          zgt_v(txPt->semno);
    }
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  zgt_p(0);   // lock tx manager
  zgt_hlink *curOwner = ZGT_Ht->find(sgno1, obno1);   // fetch link node of the Tx that is holding object

  if (curOwner != NULL) {   // if someone is holding object
    if (curOwner->tid == tid1) {  // object is locked by same transaction

      if (lockmode1 == 'X')
        curOwner->lockmode = lockmode1;
        
      this->perform_readWrite(tid1, obno1, lockmode1);    // do the actual read/write action
      zgt_v(0);
    } else {    // object is locked by different transaction
      if (lockmode1 == 'S' && curOwner->lockmode == 'S') {    // concurrent execution of read only Txs
        this->perform_readWrite(tid1, sgno1, lockmode1);    // do the actual read/write action
        zgt_v(0);
        return 0;
      }

      this->status = TR_WAIT;   // change the status to wait
      this->setTx_semno(curOwner->tid, curOwner->tid);    // set semaphore value of current tx
      this->perform_readWrite(tid1, obno1, lockmode1);    // do the actual read/write action

      zgt_v(0);   // release Tx manager
      zgt_p(curOwner->tid);   // put Tx in wait queue
      this->status = TR_ACTIVE;   // when Tx wakes up, set status to active
      this->set_lock(tid1, sgno1, obno1, count, lockmode1);   // recurse the set_lock method to again acquire the lock
    }
  } else {  // there is no owner
    this->status = TR_ACTIVE;   // set transaction status to active
    ZGT_Ht->add(this, sgno1, obno1, lockmode1);   // add tx entry to hash table
    zgt_v(0);   // release tx manager
    this->perform_readWrite(tid1, obno1, lockmode1);    // do the actual read/write action
  }
...
void zgt_tx::perform_readWrite(long tid,long obno, char lockmode){
  ...
  char txCurStatus = this->get_status();
  char *lockStatus = "Granted";
  char *lockType = "ReadLock";
  char *opType = "Read";

  if (txCurStatus == 'W')   // if the Tx is in wait mode
      lockStatus = "Not Granted";
  else {    // else increament/ decrement object value according to operation
  ...
  // sleep operation for given optime
  int x = 0;
  int i = optime;

  while(i--)
    x += rand() % 10;   // perform random operation

  // output to log file
  open_logfile_for_append();
  fprintf(logfile, "T%d\t\t%-5s    \t%d:%d:%d\t\t\t%-10s\t%-11s\t%c\n", tid, opType, obno, obval, optime, lockType, lockStatus, txCurStatus);	    // Write log record and close
  fflush(logfile);
